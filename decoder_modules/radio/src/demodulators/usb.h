...
namespace demod {
    ...
    public:
        ...
        void init(std::string name, ConfigManager* config, dsp::stream<dsp::complex_t>* input, double bandwidth, double audioSR) {
            ...
            // Define structure
            demod.init(input, dsp::demod::SSB<dsp::stereo_t>::Mode::USB, bandwidth, getIFSampleRate(), agc.attack / getIFSampleRate(), agc.decay / getIFSampleRate());
            agc.apply(demod);
        }
        ...
        void showMenu() {
            agc.showMenu(demod, this, "usb", getIFSampleRate(), ImGui::GetContentRegionAvail().x);
        }
