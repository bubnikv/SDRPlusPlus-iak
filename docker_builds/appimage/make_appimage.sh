#!/bin/bash
# Build a portable SDRIAK AppImage.
# Run inside the docker_builds/appimage Dockerfile. Source tree must be
# bind-mounted at /root/sdriak.
# Output: /root/sdriak-<arch>.AppImage
set -euo pipefail

SRC_DIR="${SRC_DIR:-/root/sdriak}"
ARCH="$(uname -m)"
APP_NAME="sdriak"
APPDIR="/AppDir"
...
# ---------------------------------------------------------------------------
# Configure SDRIAK through the root CMake project and let OPT_BUILD_DEPS drive
...
# ---------------------------------------------------------------------------
# linuxdeploy may create AppRun as a symlink to usr/bin/sdriak. Remove it
...
SDRIAK relies on the host's libglfw3 because GLFW links against your
